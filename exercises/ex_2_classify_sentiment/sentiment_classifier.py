...
class SentimentClassifier():
    ...
    def __init__(self, wdim, hdim, vocab_size, num_labels):
        ...
        # define placeholders
        review  = tf.placeholder(tf.int32, [None, None], name='review')
        labels = tf.placeholder(tf.int32, [None, ], name='label'  )

        # expose placeholders
        self.placeholders = { 'review' : review, 'label' : labels }

        # infer dimensions of batch
        batch_size_, seq_len_ = tf.unstack(tf.shape(review))

        # actual length of sequences considering padding
        seqlens = tf.count_nonzero(review, axis=-1)

        # word embedding
        wemb = tf.get_variable(shape=[vocab_size-2, wdim], dtype=tf.float32,
        ...
        with tf.variable_scope('encoder') as scope:
            _ , (fsf, fsb) = tf.nn.bidirectional_dynamic_rnn( 
                    ...
                    tf.nn.rnn_cell.LSTMCell(hdim), 
                    inputs=tf.nn.embedding_lookup(wemb, review), 
...
def rand_exec(model):
    with tf.Session() as sess:
        ...
        return sess.run(model.out,
                feed_dict = {
                    model.placeholders['review']  : np.random.randint(0, 10, [8, 10]),
...
if __name__ == '__main__':
    ...
    model = SentimentClassifier(10, 10, 10, 10)
    out = rand_exec(model)
