...
class NameGenerator():
    ...
    def __init__(self, lemb_dim, wemb_dim, hdim, vocab_size, num_labels, max_seq_len):
        ...
        # word embedding
        lemb = tf.get_variable(shape=[num_labels, lemb_dim], dtype=tf.float32,               
                        initializer=tf.random_uniform_initializer(-0.01, 0.01),          
                        name='word_embedding')

        wemb = tf.get_variable(shape=[vocab_size, wemb_dim], dtype=tf.float32,               
                        initializer=tf.random_uniform_initializer(-0.01, 0.01),          
                        name='label_embedding')

        wemb = tf.concat([ tf.zeros([1, wemb_dim]), wemb ], axis=0)
